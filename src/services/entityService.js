...
export const getEntities = () => {
  const urlGetEntities =
    process.env.REACT_APP_BASE_URL_APIREST + '/api/resource';

  return fetch(urlGetEntities).then((res) => res.json());
};

export const getEntity = (entityId) => {
  const urlGetEntity =
    process.env.REACT_APP_BASE_URL_APIREST + '/api/resource/' + entityId;

  return fetch(urlGetEntity).then((res) => res.json());
};
